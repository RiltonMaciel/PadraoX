...
function preverProximosBrancos(quantidade) {
  ...
  if (idxBrancos.length < 2) return { erro: 'Poucos brancos para calcular padrão' };
  ...
  // Ajustar hora do último branco para o tempo real atual
  const horaFonte = horas[ultimoBrancoIdx] || '--:--';
  const horaUltimoDado = horas[horas.length - 1] || '--:--';
  let horaAjustada = horaFonte;
  if (horaFonte !== '--:--' && horaUltimoDado !== '--:--') {
    const agoraReal = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' });
    const [rh, rm] = agoraReal.split(':').map(Number);
    const [dh, dm] = horaUltimoDado.split(':').map(Number);
    const delta = (rh * 60 + rm) - (dh * 60 + dm);
    if (Math.abs(delta) > 2) {
      const [fh, fm] = horaFonte.split(':').map(Number);
      const tot = ((fh * 60 + fm) + delta + 1440) % 1440;
      horaAjustada = String(Math.floor(tot / 60)).padStart(2, '0') + ':' + String(tot % 60).padStart(2, '0');
    }
  }
