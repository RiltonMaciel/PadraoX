  ...
  if (botState.apostaAberta && state.historicoGlobal.length > 0) {
    ...
    if (currentEnd !== aposta.fingerprint) {
      ...
      if (matchIdx >= 0) {
        ...
        const idxBranco = rodadasNovas.indexOf(0);
        const brancoApareceu = idxBranco !== -1;

        if (brancoApareceu) {
          // GREEN! Branco saiu — em quantas casas?
          const casas = idxBranco + 1; // 1-indexed (branco na 1ª rodada = casas 1)
          resolverAposta('green', casas);
        } else if (rodadasDesdeAposta >= aposta.rodadasParaVerificar) {
          // RED — branco não saiu no prazo
          resolverAposta('red', rodadasDesdeAposta);
        } else {
          // Atualizar contagem
        ...
        if (Date.now() - aposta.timestamp > 4 * 60 * 1000) {
          resolverAposta('red', aposta.rodadasParaVerificar);
        }
      }
    }
  }
...
// casas = quantas rodadas apostou até o resultado
function resolverAposta(tipo, casas) {
  ...
  if (tipo === 'green') {
    // Apostou em cada rodada: gastou stake × casas
    // Ganhou na última: recebeu stake × 14
    // Lucro líquido = stake × (14 - casas)
    const custoTotal = stake * casas;
    const ganho = stake * 14;
    const lucro = ganho - custoTotal; // pode ser negativo se casas > 14!
    ...
    botState.historico.push({
      ...
      frase: fraseAleatoria(lucro > 0 ? 'euforia' : 'alivio'),
      stake,
      casas,
      custoTotal,
      ganho,
    ...
    // Apostou em cada rodada e branco nunca saiu
    // Perda total = stake × casas
    const perdaTotal = stake * casas;
    botState.banca -= perdaTotal;
    botState.totalRed++;
    botState.sequenciaLoss++;
    botState.lucroTotal -= perdaTotal;
    ...
    botState.historico.push({
      hora, tipo: 'red', valor: -perdaTotal,
...
function registrarResultado(tipo, casas) {
  const agora = new Date();
  const hora = agora.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' });
  const stake = botState.apostaAberta?.stake || botState.ultimaDecisao?.stake || 5;
  casas = casas || botState.apostaAberta?.rodadasVistas || 1;

  if (tipo === 'green') {
    // Apostou em cada rodada: gastou stake × casas, ganhou stake × 14
    const custoTotal = stake * casas;
    const ganho = stake * 14;
    const lucro = ganho - custoTotal;
    botState.banca += lucro;
    botState.totalGreen++;
    botState.sequenciaLoss = 0;
    botState.galeAtivo = false;
