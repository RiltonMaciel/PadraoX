...
// ========== PREVISÃO POR TEMPO — Teorias Combinadas ==========
function analisarPrevisaoTempo() {
  ...
  // Gerar previsões a partir dos últimos N brancos
  const previsoes = [];
  const maxBrancos = Math.min(idxBrancos.length, 10); // usar até 10 últimos brancos
  ...
  for (let bi = idxBrancos.length - 1; bi >= idxBrancos.length - maxBrancos && bi >= 0; bi--) {
    ...
    // Vizinhos (4 de cada lado, pulando brancos)
    const vizAntes = [];
  ...
  // Minutos passados desde último branco
...
app.get('/api/previsao-tempo', (req, res) => {
  const resultado = analisarPrevisaoTempo();
  res.json(resultado);
});
